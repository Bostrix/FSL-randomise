...
void checkInput(const int64_t st,const  Matrix& dm,const  Matrix& tc,const  Matrix& fc) {
...
void Initialise(ranopts& opts, volume<float>& mask, Matrix& datam, Matrix& tc, Matrix& dm, Matrix& fc, Matrix& gp, Matrix& effectiveDesign)
{
  ...
  if ( opts.randomSeed.set() ) urbg.seed(opts.randomSeed.value() );
...
int main(int argc,char *argv[]) {
  ...
  try {
    ...
    if ( opts.featMode.value() || opts.outputGlm.value() ) {
      Matrix pe(model.Ncols(),data.Ncols()), cope(Tcontrasts.Nrows(),data.Ncols()), varcope(Tcontrasts.Nrows(),data.Ncols()), sigmaSquared(1,data.Ncols());
	if (voxelwiseInput.isSet)
...
void Permuter::nextShuffle(ColumnVector& perm){
   ...
   if (isRandom) shuffle(temp.begin(),temp.end(), urbg);
